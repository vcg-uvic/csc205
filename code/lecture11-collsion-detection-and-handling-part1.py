...
def process_collision(obj_list):
    ...
    for pair in collision:
        ...
        # Momentum preservation. Momentum is mass * velocity.  Before and after
        # collision we will make sure to preserve momentum.
        va_i = obj_list[pair[0]].velocity
        ma = obj_list[pair[0]].mass
        vb_i = obj_list[pair[1]].velocity
        mb = obj_list[pair[1]].mass

        # following the equations for elasitc collision
        va_f = (ma - mb) / (ma + mb) * va_i + (2 * mb) / (ma + mb) * vb_i
        vb_f = (2 * ma) / (ma + mb) * va_i + (mb - ma) / (ma + mb) * vb_i

        obj_list[pair[0]].velocity = va_f
        obj_list[pair[1]].velocity = vb_f

        # Now move according to the new velocity
        for idx in pair:
            obj_list[idx].update()
        ...
        if not init:
            ...
            obj_list = [
                ...
                block((50, 70), 100, 6, (0, 200, 200),
                      mass=10000000000000000, acceleration=(0, 0))
            ]
            init = True
